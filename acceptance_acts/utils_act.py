...
async def documents_list_async(token: str, title: str, days_back: int = 30)-> pd.DataFrame:
    ...
    Args:
        ...
        title: ID категории документов (опционально)
        days_back: Количество дней назад для периода выборки
        locale: Язык поля категории ('ru', 'en', 'zh')
        sort: Поле для сортировки ('date', 'category')
        order: Порядок сортировки ('desc', 'asc')
    
    Returns:
        DataFrame с документами или None в случае ошибки
    """
    ...
    if all_documents:
        ...
        print(f"Успешно получено {len(df)} документов за период {beginTime} - {endTime}")
        return df
    else:
...
async def get_all_fbs_acts_async():
    ...
    for account, doc_list in acceptance_certificate_dict.items():
        ...
        acts = await get_decoded_acts(account, doc_list, tokens=load_api_tokens())
    ...
    if not fbs_acts_df.empty:
        fbs_acts_df = fbs_acts_df.astype(object)
        fbs_acts_df = fbs_acts_df.where(pd.notnull(fbs_acts_df), None)  # NaN → None
        
        # ИСПРАВЛЕНО: правильное преобразование даты с dayfirst=True
        fbs_acts_df['date'] = pd.to_datetime(fbs_acts_df["date"], dayfirst=True, errors='coerce').dt.date
        
        # ЯВНОЕ приведение типов для КАЖДОЙ колонки
        fbs_acts_df['num'] = pd.to_numeric(fbs_acts_df['num'], errors='coerce').fillna(0).astype(int)
        fbs_acts_df['order_number'] = fbs_acts_df['order_number'].astype(str)
        fbs_acts_df['unit'] = fbs_acts_df['unit'].astype(str)
        fbs_acts_df['sticker'] = fbs_acts_df['sticker'].astype(str)
        fbs_acts_df['quantity'] = pd.to_numeric(fbs_acts_df['quantity'], errors='coerce').fillna(0).astype(int)
        fbs_acts_df['document'] = fbs_acts_df['document'].astype(str)
        fbs_acts_df['document_number'] = fbs_acts_df['document_number'].astype(str)
        fbs_acts_df['account'] = fbs_acts_df['account'].astype(str)
